...
def request_sanitized_op(prompt: str) -> str:
    while True:
        op = input(prompt)
        if op in ["add", "subtract", "multply", "divide"]:
            return op
        else:
...
def main():
    ...
    operation = request_sanitized_op("Enter the operation (add, subtract, multiply, divide): ").strip().lower()
